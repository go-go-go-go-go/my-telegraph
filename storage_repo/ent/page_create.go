...
func (pc *PageCreate) defaults() {
	...
	if _, ok := pc.mutation.AuthorName(); !ok {
		v := page.DefaultAuthorName
		pc.mutation.SetAuthorName(v)
	}
	if _, ok := pc.mutation.AuthorURL(); !ok {
		v := page.DefaultAuthorURL
		pc.mutation.SetAuthorURL(v)
	}
	if _, ok := pc.mutation.ImageURL(); !ok {
		v := page.DefaultImageURL
...
func (pc *PageCreate) check() error {
	...
	if _, ok := pc.mutation.AuthorURL(); !ok {
		return &ValidationError{Name: "author_url", err: errors.New(`ent: missing required field "Page.author_url"`)}
	}
	if _, ok := pc.mutation.ImageURL(); !ok {
